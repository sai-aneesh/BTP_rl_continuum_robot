...
void ForceModelAssembler::GetTangentStiffnessMatrixTopology(SparseMatrix ** tangentStiffnessMatrix)
{
  *tangentStiffnessMatrix = new SparseMatrix(*Ktemplate);
  ...
  GetEnergyAndForceAndMatrix(zeros.data(), nullptr, nullptr, *tangentStiffnessMatrix,nullptr);
}

void ForceModelAssembler::GetEnergyAndForceAndMatrix(const double * u, double * energy, double * internalForces, SparseMatrix * tangentStiffnessMatrix, SparseMatrix * massMatrix)
{
  // reset to zero
  if (internalForces)
...
// in default setting else part is used
#ifdef USE_TBB
  ...
  for (int eltype = 0; eltype < stencilForceModel->GetNumStencilTypes(); eltype++) 
  {
    int nelev = stencilForceModel->GetNumStencilVertices(eltype);
    int nele = stencilForceModel->GetNumStencils(eltype);
    // cout<<"vertices :"<<nelev<<" GetNumStencils :"<<nele<<endl;
...
{
  double E = 0;
  GetEnergyAndForceAndMatrix(u, &E, nullptr, nullptr,nullptr);
...
{
  GetEnergyAndForceAndMatrix(u, nullptr, internalForces, nullptr,nullptr);
}

void ForceModelAssembler::GetTangentStiffnessMatrix(const double * u, SparseMatrix *tangentStiffnessMatrix)
{
  GetEnergyAndForceAndMatrix(u, nullptr, nullptr, tangentStiffnessMatrix,nullptr);
}

void ForceModelAssembler::GetForceAndMatrix(const double * u, double * internalForces, SparseMatrix * tangentStiffnessMatrix, SparseMatrix * massMatrix)
{
  GetEnergyAndForceAndMatrix(u, nullptr, internalForces, tangentStiffnessMatrix,massMatrix);
}
