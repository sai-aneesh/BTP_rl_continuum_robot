...
MassSpringSystem::MassSpringSystem(int numParticles_, double * restPositions_, int numQuads, int * quads, double surfaceDensity, double tensileStiffness, double shearStiffness, double bendStiffness, double damping, int addGravity_): addGravity(addGravity_), g(9.81) // creates the mass spring system from a quad surface mesh (e.g., cloth)
{
  ...
  for(int i=0; i<numQuads; i++)
  {
    ...
    masses_[v0] += 0.25 * surfaceArea * surfaceDensity;
    masses_[v1] += 0.25 * surfaceArea * surfaceDensity;
    masses_[v2] += 0.25 * surfaceArea * surfaceDensity;
    masses_[v3] += 0.25 * surfaceArea * surfaceDensity;

  }

  //redefining the way spring edges and the groupStiffnes is defined
  int numMaterialGroups_ = numQuads;
  double * groupStiffness_ = (double*) malloc (sizeof(double) * numMaterialGroups_);
  int numEdges_ = 6*numQuads;
  ...
  for(int i=0; i<numQuads; i++)
  {
    ...
    edges_[2*count+0] = v0;
    edges_[2*count+1] = v1;
    edgeGroups_[count] = floor(count*0.25); // tensile group
    count++;
  ...
  for(int i=0; i<numQuads; i++)
  {
    ...
    edges_[2*count+0] = v0;
    edges_[2*count+1] = v2;
    edgeGroups_[count] = floor((count-numQuads*4)*0.5); // shear group
    count++;
  ...
  //   cout<<"Spring for edge:"<<edges_[2*i]<<" and "<<edges_[2*i+1]<<" the grp group alloted is:"<<edgeGroups_[i]<<endl;
  ...
  cout<<"MAss spring model generated."<<endl;
  free(masses_);
  free(edges_);
  free(edgeGroups_);
