...
class vega_env(gym.Env):
    ...
    def __init__(self, n_sections):
        ...
        t_x =  0.3*np.sin(self.theta)
        t_y =  0.3*(1-np.cos(self.theta))
...
if __name__ == '__main__':
    ...
    model = SAC.load("/home/aneesh/catkin_ws/src/Soft_robo_sim/vega_simulator/config/logs/Feb8_SAC_moving_goal_10000_steps", tensorboard_log="/home/aneesh/catkin_ws/src/Soft_robo_sim/vega_simulator/results")
    model.set_env(env)

    # Save a checkpoint every 5000 steps
    log_dir = os.getcwd()
    # print(log_dir)
    checkpoint_callback = CheckpointCallback(save_freq=5000, save_path=log_dir +'/logs/',name_prefix='Feb8_SAC_moving_goal')
    ...
    model.learn(total_timesteps=int(3e4), tb_log_name = "Feb8_SAC_moving_goal", callback=callback)

    # # save the model
    model.save(log_dir + "/Feb8_SAC_moving_goal")

    # load the trained model
    model = SAC.load("/home/aneesh/catkin_ws/src/Soft_robo_sim/vega_simulator/config/Feb8_SAC_moving_goal")    
    # Save the policy independently from the model
    # Note: if you don't save the complete model with `model.save()`
    # # you cannot continue training afterward
    policy = model.policy
    policy.save("Feb8_SAC_moving_goal")
