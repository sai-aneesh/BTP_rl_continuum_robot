...
class vega_env(gym.Env):
    ...
    def reset(self):
        ...
        # rospy.loginfo(self.target)
        if(self.theta <= 1.5) and (self.clck == False):
            self.theta += 0.02
        elif(self.theta > 1.5) and (self.clck == False):
            self.theta -= 0.02
            self.clck = True
        elif(self.theta >= -1.5) and (self.clck == True):
            self.theta -= 0.02
        elif (self.theta < -1.5) and (self.clck == True):
            self.theta += 0.02
            self.clck = False
...
if __name__ == '__main__':
    ...
    # log_dir = os.getcwd()
    # # print(log_dir)
    # checkpoint_callback = CheckpointCallback(save_freq=5000, save_path=log_dir +'/logs/',name_prefix='Feb10_Extend_moving_goal')
    ...
    # model.learn(total_timesteps=int(3e4), tb_log_name = "Feb10_Extend_moving_goal", callback=callback)

    # # # save the model
    # model.save(log_dir + "/Feb10_Extend_moving_goal")

    # load the trained model
    model = SAC.load("/home/aneesh/catkin_ws/src/Soft_robo_sim/vega_simulator/config/Feb10_Extend_moving_goal")    
    # Save the policy independently from the model
    # Note: if you don't save the complete model with `model.save()`
    # # you cannot continue training afterward
    # policy = model.policy
    # policy.save("Feb10_Extend_moving_goal")
